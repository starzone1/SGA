...
export default async function handler(req, res) {
  ...
  // Static baseline data fallbacks (same as sitemap.js and initialData.ts)
  const fallbackUsers = [
  ...
  if (segments.length === 1) {
    ...
    if (nonProfilePaths.includes(s1.toLowerCase())) {
      ...
      const matchedAuthor = users.find(u => createSlug(u.name) === s1 || createSlug(u.id) === s1);
      if (matchedAuthor) {
    ...
    if (s1 === 'kategori') {
      ...
      if (matchedArticle) {
        ...
        pageDesc = matchedArticle.excerpt || `${matchedArticle.title}. Baca ulasan dan liputan jurnalisme selengkapnya secara independen hanya di SGA News.`;
